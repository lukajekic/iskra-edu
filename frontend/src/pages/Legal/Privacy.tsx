...
### Obrada podataka prilikom korišćenja Iskra AI Mentora

Unutar platforme Iskra integrisan je AI asistent koji učenicima pruža pedagoške smernice i analizu koda. Kako bismo osigurali maksimalnu privatnost naših korisnika, ovaj sistem funkcioniše po sledećim principima:

-   **Potpuna anonimizacija koda:** Kada učenik zatraži pomoć od AI mentora, sistem eksternom procesoru (AI modelu) prosleđuje isključivo tekst zadatka i programski kod koji je učenik napisao. **Nijedan lični podatak** (ime, prezime, ID korisnika) se ne šalje eksternim servisima.
    
-   **Korišćenje eksternih API servisa:** Za generisanje tekstualnih sugestija, Iskra koristi napredne jezičke modele (poput Llama modela kompanije Meta) koji se izvršavaju putem bezbednih i optimizovanih eksternih API platformi. Ovi servisi nemaju pristup identitetu korisnika i ne mogu povezati poslati kod sa konkretnom osobom.
    
-   **Prikupljanje podataka za unapređenje platforme (Dataset):** U cilju razvoja sopstvenih edukativnih modela i poboljšanja kvaliteta sugestija na srpskom jeziku, Iskra zadržava pravo da u svojoj bazi podataka trajno sačuva parove podataka: _tekst zadatka_, _netačan kod učenika_ i _generisanu AI sugestiju_. Ovi podaci se skladište u **100% anonimizovanoj formi** i koriste se isključivo za naučno-istraživački rad i mašinsko učenje (Machine Learning) unutar Iskra platforme.
    
-   **Pravo na kontrolu:** Pošto se podaci o kodu čuvaju bez ikakve veze sa Vašim nalogom, ne postoji tehnička mogućnost da se konkretan kod poveže sa Vama, niti da se na zahtev obriše, jer sistem ne zna koji kod pripada kom učeniku.
